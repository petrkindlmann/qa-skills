...
Production-grade QA and test automation skills for Claude Code, Codex, Cursor,
...
Includes battle-tested skills for:
...
- AI-powered test generation & bug triage
- chaos engineering & observability
- coverage analysis & QA dashboards
...
39 skills across 10 categories. Each is production-tested, cross-referenced, and under 500 lines.

### Foundation (1)
`qa-project-context` — universal project context template that every other skill reads first

### Strategy (4)
`test-strategy` · `test-planning` · `risk-based-testing` · `exploratory-testing` — QA strategy creation, sprint/release test plans, risk-based prioritization, session-based exploratory testing

### Automation (7)
`playwright-automation` · `cypress-automation` · `api-testing` · `unit-testing` · `mobile-testing` · `visual-testing` · `performance-testing` — Playwright E2E, Cypress component/E2E, REST/GraphQL, Jest/Vitest/pytest, Appium/Detox, visual regression, k6 load testing & Lighthouse CI

### Specialized (4)
`accessibility-testing` · `security-testing` · `cross-browser-testing` · `database-testing` — WCAG compliance, OWASP Top 10, analytics-driven browser matrices, migration testing & schema validation

### AI-Augmented QA (4)
`ai-test-generation` · `ai-bug-triage` · `test-reliability` · `ai-qa-review` — LLM-powered test generation, automated bug triage, flaky test management, QA-focused code review
...
### Metrics (3)
`qa-metrics` · `qa-dashboard` · `coverage-analysis` — quality gates & KPIs, Allure/Grafana/ReportPortal dashboards, coverage-as-ratchet in CI
...
|-------|-------------|----------|
| [`qa-project-context`](skills/qa-project-context) | Universal project context template — tech stack, test frameworks, CI/CD, environments, quality goals. Every other skill reads this first. | Foundation |
| [`test-strategy`](skills/test-strategy) | Full QA strategy creation with risk-based prioritization, test pyramid design, entry/exit criteria, and tool selection rationale. | Strategy |
...
| [`performance-testing`](skills/performance-testing) | Performance testing with k6 load/stress/soak scripts, Lighthouse CI for Web Vitals, and performance budgets as CI gates. | Automation |
...
| [`ai-test-generation`](skills/ai-test-generation) | LLM-powered test generation from specs, PRDs, and user stories. Prompt patterns for edge case discovery and assertion generation. | AI-QA |
| [`ai-bug-triage`](skills/ai-bug-triage) | Auto-classify bugs by severity/component/root-cause, deduplicate issues, analyze CI failures, suggest fix priorities. | AI-QA |
| [`test-reliability`](skills/test-reliability) | Locator resilience strategies, auto-repair patterns, selector stability scoring, flaky test detection and quarantine. | AI-QA |
| [`ai-qa-review`](skills/ai-qa-review) | QA-focused code review — test quality assessment, coverage gap detection, anti-pattern detection. | AI-QA |
...
| [`test-data-management`](skills/test-data-management) | Test data strategies — factories, fixtures, synthetic data generation, database seeding, data cleanup, environment isolation. | Infrastructure |
...
| [`qa-metrics`](skills/qa-metrics) | Essential QA metrics — coverage %, flakiness rate, defect escape rate, MTTR, test execution trends, quality gates. | Metrics |
...
| [`ai-system-testing`](skills/ai-system-testing) | Test AI-powered features — LLM prompt regression, tool call validation, nondeterministic output evaluation, hallucination risk assessment. | Knowledge |
...
Most QA skills repos cover test frameworks. This one also covers how AI agents can actively improve your testing.

| Skill | What it does |
|-------|-------------|
| `ai-test-generation` | Generate test cases from PRDs, specs, and user stories using LLM prompt patterns |
| `ai-bug-triage` | Auto-classify bugs by severity/component/root-cause, deduplicate issues, analyze CI failures |
| `test-reliability` | Auto-detect broken locators, score selector stability, quarantine flaky tests |
| `ai-qa-review` | QA-focused code review — test quality assessment, coverage gap detection, anti-pattern detection |

These four skills represent the future of QA — where the AI agent isn't just executing your instructions but actively finding gaps, generating coverage, and maintaining test health.

---

## Compatibility

These skills work with any tool that supports the [Agent Skills Standard](https://agentskills.io).
...
Skills reference specific QA tools for implementation. The [Tools Registry](tools/REGISTRY.md) catalogs all supported tools with:

- **Capabilities** — What each tool does and when to use it
- **MCP server availability** — Which tools have Model Context Protocol servers for direct agent integration
- **Integration guides** — Per-tool setup and configuration docs

Covered categories: test frameworks, reporting, visual testing, CI/CD, project management, security scanning, and observability.

---

## Project Context

Every skill checks for `.agents/qa-project-context.md` before asking discovery questions. This file captures your project-specific QA context:

- Tech stack and test frameworks
- CI/CD pipeline configuration
- Environment details (dev, staging, production)
- Quality goals and coverage targets
- Risk areas and team structure

Create it once using the `qa-project-context` skill. All other skills read it automatically — no repeated questions about your stack.

---

## Contributing

See [CONTRIBUTING.md](CONTRIBUTING.md) for guidelines on adding new skills, improving existing ones, and adding tool integrations.
...
<sub>This project is an independent open-source collection of agent skills.
It is not affiliated with, endorsed by, or sponsored by Microsoft,
Cypress, Grafana Labs, BrowserStack, Sauce Labs, or any other
referenced vendor. Product names, logos, and brands are the property
